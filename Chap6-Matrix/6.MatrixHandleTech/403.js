/**
 * Problem: Rotate 180 degrees the matrix
 *
 *
 *
 *
...
function fx1(m) {
  /**
   *   + Rotate 180 degrees at top left corner m[0][0]
   *
   *   + Right rotate
   *   - Matrix           - ret
   *   -- 0 1 2           -- 0 1 2
   ...
   *   3| 1 0 1           3| 1 2 3
   *
   *  + step 1: generate empty matrix, m.length rows, m[0].length
   *  column
   *  + step 2: column traverse, pour matrix
   *
   *
   */
  ...
  for (let i = m[0].length - 1; i >= 0; --i) {
    for (let j = m.length - 1; j >= 0; --j) {
      matrixRet[m.length - j - 1][i] = m[j][i];
    }
  }
...
function fx3(m) {
  /**
   *
   * + Rotate 180 degrees at top right corner m[0][m[0].length-1]
   *   + Right rotate
   *   - Matrix           - ret
   *   -- 0 1 2           -- 0 1 2
   *   0| 1 2 3           0| 1 0 1
  ...
  const columnMidIndex = Math.floor(m[0].length / 2);

  for (let i = m[0].length - 1; i >= columnMidIndex; --i) {
    for (let j = m.length - 1; j >= 0; --j) {
      const temporary = m[j][i];
