/**
 * Problem: Remove a row in the matrix
 * 
 * 
 ...
 * 2| 7 11 13
 * 
 * Approach 1: pour matrix
 * Approach 2: translate, length--
 * 
 * 
 * 
 * /
 ...
 * 
 */
function fx1(m, remove_row_index) {
  let ret = new Array(m.length - 1);

  for (let i = 0; i < remove_row_index; ++i) {
    ret[i] = m[i];
  }

  for (let j = remove_row_index + 1; j <= m.length - 1; ++j) {
    ret[j - 1] = m[j];
  }
...
function test1() {
  ...
  advanceLogMatrix(m2);
  console.log("remove_row_index_0: ", remove_row_index_0);
  advanceLogMatrix(fx1(m2, remove_row_index_0));

  advanceLogMatrix(m4);
  console.log("remove_row_index_0: ", remove_row_index_1);
  advanceLogMatrix(fx1(m4, remove_row_index_1));

  advanceLogMatrix(m5);
  console.log("remove_row_index_0: ", remove_row_index_2);
  advanceLogMatrix(fx1(m5, remove_row_index_2));
}
