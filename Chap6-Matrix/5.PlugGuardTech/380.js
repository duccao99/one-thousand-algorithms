/**
 * Problem: Count the number of smallest even value 
...
function fx(m) {
  ...
  for (let i = m.length - 1; i >= 0; --i) {
    for (let j = m[i].length - 1; j >= 0; --j) {
      if (isEvenNumber(m[i][j]) && m[i][j] < smallestEvenValue) {
        smallestEvenValue = m[i][j];
      }
    }
  }
  let ret = {
  ...
  function generateMatrix(rows, columns) {
    ...
    for (let i = rows - 1; i >= 0; --i) {
      ...
      for (let j = columns - 1; j >= 0; --j) {
        row = push(row, generateNumber(0, 9));
      }
      ret = push(ret, row);
    }
    return ret;
