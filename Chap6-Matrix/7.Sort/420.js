/**
 * Problem: Sort the elements those lying on the matrix boundary
 ...
 * 2| 9 1 4 5
 *
 * - array boundary elements = [4,1,3,5,8,5,4,1,9,2]
 * - array boundary elements sorted in ascendng order
 * = [1, 1, 2, 3, 4, 4, 5, 5, 8, 9]
 *
 *
 *
 * - boundary sorted in ascending order clockwise
 * -- 0 1 2 3
 * 0| 1 1 2 3
 * 1| 9     4
 * 2| 8 5 5 4
 ...
 *
 * Appoach
 * + step 1: get array element of boundary
 * + step 2: sort step 1 in ascending order
 * + step 3: use boundary traverse tech, fill step 2
 *
 *
 */
  ...
  for (let i = 0; i <= m.length - 1; ++i) {
    if (i === 0) {
      for (let j = 0; j <= m[i].length - 1; ++j) {
        boundaryElements = push(boundaryElements, m[i][j]);
      }
    }
    if (i > 0 && i < m.length - 1) {
      boundaryElements = push(boundaryElements, m[i][m[i].length - 1]);
    }
    if (i === m.length - 1) {
      ...
      for (let k = m.length - 1; k > 0; --k) {
        boundaryElements = push(boundaryElements, m[k][0]);
      }
    }
  }

  boundaryElements = ascendingBubbleSort(boundaryElements);

  for (let i = 0; i <= m.length - 1; ++i) {
    if (i === 0) {
      for (let j = 0; j <= m[i].length - 1; ++j) {
        m[i][j] = boundaryElements[0];
        boundaryElements = shift(boundaryElements);
      }
    }
    if (i > 0 && i < m.length - 1) {
      m[i][m[i].length - 1] = boundaryElements[0];
      boundaryElements = shift(boundaryElements);
    }
    if (i === m.length - 1) {
      for (let j = m[i].length - 1; j >= 0; --j) {
